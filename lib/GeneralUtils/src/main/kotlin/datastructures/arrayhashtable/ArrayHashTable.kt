 ...
 */
abstract class ArrayHashTable(val container: ArrayHashTableContainer, val capacity: Int) {
    //
    ...
    internal val bucketCount
        get() = 1 shl bucketCountLog2
    ...
    //
    @PublishedApi internal val keys = arrayOfNulls<Any?>(capacity + 1)
    @PublishedApi internal val values = objectArrayOrEmpty(container.hasValues, capacity + 1)
    ...
    abstract fun _computeValueIfAbsent(key: Any?, f: (Any?) -> Any?): Any?
    abstract fun _computeValueOrderedIfAbsent(key: Any?, f: (Any?) -> Any?): Any?
 ...
 */
fun createArrayHashTable(container: ArrayHashTableContainer, initialCapacity: Int): ArrayHashTable {
...
internal inline fun <T : ArrayHashTable> T.computeValueIfAbsent(key: Any?, f: (Any?) -> Any?): Any? {
    addKey(key) { added, i ->
        // Java's computeIfAbsent treats null values as absent, so we do the same.
        val oldV = if (added) { null } else { values[i] }
        if (oldV == null) {
            val v = f(key)
            values[i] = v
            return v
...
internal inline fun <T : ArrayHashTable> T.computeValueOrderedIfAbsent(key: Any?, f: (Any?) -> Any?): Any? {
    addKey(key) { added, i ->
        // Java's computeIfAbsent treats null values as absent, so we do the same.
        val oldV = if (added) { null } else { values[i] }
        if (oldV == null) {
